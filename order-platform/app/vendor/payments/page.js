...
import { CheckCircle, XCircle, Clock, X, AlertCircle } from 'lucide-react';
...
export default function VendorPayments() {
  ...
  const loadPayments = async () => {
    ...
    const { data, error } = await supabase
      ...
      .in('status', ['payment_pending', 'payment_received', 'customer_accepted', 'processing', 'completed', 'delivered'])
  ...
  const pendingPayments = orders.filter(o => o.status === 'payment_pending' && o.payment_status !== 'paid');
  ...
  return (
    <DashboardLayout userType="vendor" userData={userData}>
      ...
      {/* Receipt Modal - Now shows actual image */}
      {selectedOrder && (
              ...
              <div className="border border-gray-200 rounded-lg p-4">
                ...
                {imageError && (
                  <div className="bg-red-50 p-4 rounded-lg flex items-start">
                    ...
                    <div>
                      <p className="text-red-700 font-medium">Failed to load receipt image</p>
                      <p className="text-sm text-red-600 mt-1">The image may have expired or the bot token is not configured.</p>
                    </div>
                  </div>
                )}
                
                <img 
                  src={`/api/telegram-image?file_id=${selectedOrder.payment_receipt_url}`}
                  alt="Payment Receipt"
                  className={`w-full rounded-lg ${imageLoading ? 'hidden' : ''} ${imageError ? 'hidden' : ''}`}
